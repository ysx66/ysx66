...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/ao889/common build/common
...
git clone -b main --depth 1 https://github.com/ao889/OpenWrt shangyou
...
do
  ...
  if [[ "${SOURCE_CODE1}" == "COOLSNOWWOLF" ]]; then
...
if [[ "${GIT_REPOSITORY}" =~ (ao889/OpenWrt) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/ao889/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/ao889/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="ao889/OpenWrt"
  sudo rm -rf build common.sh
